// src/routes/theses.tsx
// Thèses — données Firebase Firestore + téléchargement via Firebase Storage URL
...
function ThesesPage() {
  ...
  async function handleDownload(pub: Publication) {
    ...
    if (pub.fileURL) {
      ...
      setTheses((prev) =>
        prev.map((t) => t.id === pub.id ? { ...t, downloads: t.downloads + 1 } : t),
      )
    } else {
    ...
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      >
        ...
        <div className="text-sm">
          <strong className="text-[#c9a227]">Accès libre et gratuit</strong> — La bibliothèque des thèses
          de l'École SCIRE est entièrement ouverte au public conformément à la politique d'accès libre
      ...
      <div className="mb-8">
        <input
          type="text"
          placeholder="Rechercher une thèse, un auteur ou un thème…"
          value={search}
      ...
      {loading ? (
        <div className="text-center py-16 text-gray-400">Chargement des thèses…</div>
      ) : filtered.length === 0 ? (
        <div
        ...
        >
          <p className="text-gray-500">
            {theses.length === 0
              ? 'Aucune thèse disponible pour le moment.'
        ...
        <>
          <p className="text-sm text-gray-500 mb-6">
            {filtered.length} thèse{filtered.length !== 1 ? 's' : ''}
          </p>
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <button
                      ...
                      disabled={!thesis.fileURL}
                      className="flex-shrink-0 flex flex-col items-center gap-1 px-4 py-3 rounded font-semibold text-sm text-[#1a2a4a] disabled:opacity-40"
